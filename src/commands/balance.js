    ...
    execute: async (interaction) => {
        try {
            ...
            const responseMessage = data.newlyAdded
                ? `${interaction.user.username}님의 계좌가 존재하지 않아, 방금 생성했습니다! 
                \`${interaction.user.username}님의 초기 잔액은 ${data.balance}입니다.\``
                : `\`${interaction.user.username}님의 현재 잔액은 ${data.balance}입니다.\``;
